...
if (isset($_GET['rank_id'])) 
    ...
    $objPHPExcel = $objReader->load("templates/outstanding_trim.xls");
    
    switch ($selectMonth){
        case 1:
            $selectMonth = 'មករា-មិនា';
            break;
        case 4:
            $selectMonth = 'មេសា-មិថុនា';
            break;
        
        case 7:
            $selectMonth = 'កក្កដា-កញ្ញា';
            break;
        
        case 10:
            $selectMonth = 'តុលា-ធ្នូ';
            break;
    }
    ...
    foreach ($classes as $r => $dataRow) {
        $row = $baseRow + 1;
        $getTrimExamScore = getTopTrimScore($examID, $dataRow['class_id']);
...
//    $objPHPExcel->getActiveSheet()->removeRow($baseRow - 1, 1);
    ...
    $filename = "outstanding_trim.xls";
    $file_path = "http://" . $_SERVER['HTTP_HOST'] . "/rachana/pages/" . $filename;
//      var_dump($file_path);
    $contenttype = "application/octet-stream";
    header("Content-Type: " . $contenttype);
    header("Content-Disposition: attachment; filename=\"" . basename("outstanding_trim-" . $selectMonth . ".xls") . "\";");
