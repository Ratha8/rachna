...
if (isset($_GET['rank_id'])) 
    ...
    $objPHPExcel = $objReader->load("templates/outstanding_sem.xls");
    
    switch ($selectMonth){
        case 1:
            $selectMonth = 'Semester 1';
            break;
        case 7:
            $selectMonth = 'Semester 2';
            break;
    }
    ...
    foreach ($classes as $r => $dataRow) {
        $row = $baseRow + 1;
        $getTSemExamScore = getTopTrimScore($examID , $dataRow['class_id']);
...
//            $objPHPExcel->setActiveSheetIndex(0)->mergeCells('A' . $baseRow . ':M' . $baseRow);
            $objPHPExcel->getActiveSheet()->setCellValue('A'.$row, $number+1)
                    ...
                    ->setCellValue('F' . $row, '')
                    ->setCellValue('J' . $row, round($totalAll, 2))
...
//    $objPHPExcel->getActiveSheet()->removeRow($baseRow - 1, 1);
    ...
    $filename = "outstanding_sem.xls";
    $file_path = "http://" . $_SERVER['HTTP_HOST'] . "/rachana/pages/" . $filename;
//      var_dump($file_path);
    $contenttype = "application/octet-stream";
    header("Content-Type: " . $contenttype);
    header("Content-Disposition: attachment; filename=\"" . basename("outstanding_sem-" . $selectMonth . ".xls") . "\";");
