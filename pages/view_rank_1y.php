...
<div class="content-wrapper">
    ...
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            ...
            <li><a href="view_rank_records.php">Record List</a></li>
            ...
            <div class="col-xs-12">
                <div class="box box-success">
                    <div class="box-header">
                        <h3 class="text-center">TOP STUDENT OF THE YEAR <?php echo $rank->getRank_name() ?></h3>
                        <span class="input-group pull-right">
                        <a role="button" href="outstanding_year.php?rank_id=<?php echo $rank_id ; ?>&year=<?php echo $rank->getYear() ?>" class="btn btn-success btn-icon"
                            ...
                            <div class="table-responsive">
                                <table id="student-list" class="table table-bordered table-hover">
                                    ...
                                    <tbody>
                                        ...
                                        $where = '';
                                        if ($examID == NULL) {
                                            echo '<td colspan="10" class="text-center">NO RECORD FOR ' . $rank->getYear() . '</td>';
                                        } else {
                                            foreach ($classes as $key => $value) {
                                                $getTSemExamScore = getTopTrimScore($examID, $value['class_id']);
                                                echo '<td colspan="20" class="warning" id="class"><b>' . integerToRoman($row_num) . '- ' . $value['class_name'] . '</b></td>';
                                                if (!empty($getTSemExamScore)) {
                                                    foreach ($getTSemExamScore as $number => $record) {
                                                        $mention = "";
                                                        ...
                                                        echo '<td>
                                                            <a class="btn btn-link btn-icon" href="certi_register_outstanding.php?id='.$student->getStudentID().'&level='.$classes[$key]['level_id'].'&score='.$totalAll.'&duration=1&year='.$rank->getYear().'&type=4" role="button" 
